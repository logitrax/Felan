...
class Widget_Google_Map extends Base
{
	...
	{
		return 'felan-google-map';
	}

	public function get_title()
	{
		return esc_html__('Advanced Google Map', 'felan');
	}
	...
	{
		return ['gmap3', 'gmap-api', 'felan-widget-google-map'];
	}

	public function get_style_depends()
	{
		return ['felan-el-widget-google-map'];
	}
	...
	private function add_map_options_section()
	{
		...
		$this->add_control('style', [
			'label'   => esc_html__('Map Style', 'felan'),
		...
		$this->add_control('type', [
			'label'   => esc_html__('Map Type', 'felan'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'roadmap'   => esc_html__('Roadmap', 'felan'),
				'satellite' => esc_html__('Satellite', 'felan'),
				'hybrid'    => esc_html__('Hybrid', 'felan'),
				'terrain'   => esc_html__('Terrain', 'felan'),
			],
			'default' => 'roadmap',
		]);

		$this->add_control('zoom_level', [
			'label'       => esc_html__('Zoom Level', 'felan'),
			'placeholder' => '12',
		...
		$this->add_control('scrollwheel', [
			'label'   => esc_html__('Mouse Scroll Wheel Zoom', 'felan'),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		]);

		$this->add_control('draggable', [
			'label'   => esc_html__('Draggable', 'felan'),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		]);

		$this->add_control('map_controls_heading', [
			'label'     => esc_html__('Controls', 'felan'),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		]);

		$this->add_control('zoom_control', [
			'label' => esc_html__('Zoom Control', 'felan'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('scale_control', [
			'label' => esc_html__('Scale Control', 'felan'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('fullscreen_control', [
			'label' => esc_html__('Fullscreen Control', 'felan'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('street_view_control', [
			'label' => esc_html__('Street View Control', 'felan'),
			'type'  => Controls_Manager::SWITCHER,
		]);
	...
	private function add_map_markers_section()
	{
		$this->start_controls_section('markers_section', [
			'label' => esc_html__('Markers', 'felan'),
		]);

		$this->add_control('marker_style', [
			'label'       => esc_html__('Marker Style', 'felan'),
			'description' => esc_html__('Select default style for all markers', 'felan'),
			'type'        => Controls_Manager::SELECT,
			'options'     => [
				'icon'   => esc_html__('Icon', 'felan'),
				'signal' => esc_html__('Signal', 'felan'),
			],
			'default'     => 'icon',
		]);

		$this->add_control('marker_icon', [
			'label'     => esc_html__('Marker Icon', 'felan'),
		...
		$repeater->start_controls_tab('marker_content_tab', [
			'label' => esc_html__('Marker', 'felan'),
		]);

		$repeater->add_control('address', [
			'label'       => esc_html__('Address or Coordinate', 'felan'),
		...
		$repeater->start_controls_tab('marker_popup_tab', [
			'label' => esc_html__('Popup', 'felan'),
		]);

		$repeater->add_control('overlay_display', [
			'label'   => esc_html__('Display', 'felan'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''       => esc_html__('Hover', 'felan'),
				'always' => esc_html__('Always', 'felan'),
			],
			'default' => '',
		]);
		...
		$repeater->add_control('image', [
			...
			'description' => esc_html__('Choose an image that displays on left info box', 'felan'),
			'type'        => Controls_Manager::MEDIA,
			'classes'     => 'felan-control-media-auto',
		]);
		...
		$this->add_group_control(Group_Control_Image_Size::get_type(), [
			'label'     => esc_html__('Popup Image Size', 'felan'),
	...
	private function add_map_markers_style_section()
	{
		$this->start_controls_section('markers_popup_style_section', [
			'label' => esc_html__('Markers Popup', 'felan'),
		...
		$this->add_responsive_control('markers_popup_padding', [
			...
			'size_units' => ['px', '%', 'em'],
			'selectors'  => [
				'{{WRAPPER}} .felan-map-overlay-info' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		]);
		...
		$this->add_responsive_control('markers_popup_width', [
			...
			'selectors'  => [
				'{{WRAPPER}} .felan-map-overlay-content' => 'width: {{SIZE}}{{UNIT}};',
			],
		]);
...
<?php
	...
	private function get_overlay_value($settings, $marker, $position)
	{
		...
		$this->add_render_attribute($item_key, 'class', [
			'felan-map-overlay',
			"felan-map-overlay-{$marker['overlay_display']}",
		...
		if ($marker['title'] !== '' || $marker['content'] !== '') {
			$info_template .= '<div class="felan-map-overlay-template"><div class="felan-map-overlay-content"><div class="felan-map-overlay-info">';

			if (isset($marker['image'])) {
				$size  = \Felan_Image::elementor_parse_image_size($settings, '80x9999', 'popup_image_size');
				$image = \Felan_Image::get_attachment_by_id(array(
					'id'   => $marker['image']['id'],
					'size' => $size,
				));
	...
	{
		return [
			'no_label_bright_colors'  => esc_html__('No Label Bright Colors', 'felan'),
			'grayscale'               => esc_html__('Grayscale', 'felan'),
			'subtle_grayscale'        => esc_html__('Subtle Grayscale', 'felan'),
			'apple_paps_esque'        => esc_html__('Apple Maps-esque', 'felan'),
			'pale_dawn'               => esc_html__('Pale Dawn', 'felan'),
			'midnight_commander'      => esc_html__('Midnight Commander', 'felan'),
			'blue_water'              => esc_html__('Blue Water', 'felan'),
			'retro'                   => esc_html__('Retro', 'felan'),
			'paper'                   => esc_html__('Paper', 'felan'),
			'ultra_light_with_labels' => esc_html__('Ultra Light with Labels', 'felan'),
			'shades_of_grey'          => esc_html__('Shades Of Grey', 'felan'),
			'easy_light'              => esc_html__('Easy Light', 'felan'),
			'wy'                      => esc_html__('WY', 'felan'),
		];
	}
	...
	{
		$icon_url = FELAN_ELEMENTOR_ASSETS . '/images/map-marker.png';
