...
if (!class_exists("Felan_Kirki")) {
	class Felan_Kirki
	{
					...
					if (isset($args["default"]) && is_array($args["default"])) {
						foreach ($args["default"] as $key => $default_value) {
							...
							if (is_string($subfield["output"])) {
								...
								foreach ($subfield["output"]
									as $key => $output) {
									$subfield["output"][$key]["property"] = $output_property;
								}
							}
							$type = "select";
							if (in_array($key, ["color", "image"])) {
								$type = $key;
							}
							$property_setting =
								esc_attr($args["settings"]) . "_" . $setting;
		...
		public function enqueue_styles()
		{
			...
			if (!empty($styles)) {
				wp_add_inline_style("felan-style", $styles);
			}
		}
		...
		public function get_styles()
		{
				...
				foreach ($field["output"] as $output) {
					...
					if (!is_array($value)) {
						...
						if (
							...
							$css[$output["media_query"]][$output["element"]][$output["property"]] =
								$output["prefix"] .
								$value .
								$output["units"] .
								$output["suffix"];
						...
						if ("typography" === $field["type"]) {
							foreach ($value as $key => $subvalue) {
								...
								if ("variant" === $key) {
									...
									$font_weight = in_array($font_weight, [
										"",
										"regular",
									])
										? "400"
			...
			foreach ($css as $media_query => $styles) {
				...
				$final_css .=
					"global" != $media_query ? $media_query . "{" : "";
