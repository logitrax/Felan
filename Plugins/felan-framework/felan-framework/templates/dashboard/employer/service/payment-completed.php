...
if ($service_payment_method == 1) {
    $felan_service_payment->paypal_payment_completed();
} elseif ($service_payment_method == 2) {
    $felan_service_payment->stripe_payment_completed();
} elseif ($service_payment_method == 4) {
    $felan_service_payment->razor_payment_completed();
}
?>
<div class="felan-payment-completed-wrap">
    <div class="inner-payment-completed">
        <?php
        do_action('felan_before_service_payment_completed');
        if (isset($_GET['order_id']) && $_GET['order_id'] != '') :
            $order_id = absint(wp_unslash($_GET['order_id']));
            $felan_service_order = new Felan_service_order();
            $order_meta = $felan_service_order->get_service_order_meta($order_id);
                ...
                <ul class="list-group">
                    ...
                    <li class="list-group-item">
                        ...
                        <strong class="pull-right">
                            <?php echo Felan_service_order::get_service_order_payment_method($order_meta['service_order_payment_method']);  ?>
                        </strong>
                    </li>
                    <li class="list-group-item">
                        <span><?php esc_html_e('Total', 'felan-framework'); ?></span>
                        <strong class="pull-right"><?php echo $order_meta['service_order_item_price']; ?></strong>
                    </li>
                </ul>
            </div>
        ...
        <?php else : ?>
            ...
            <a href="<?php echo felan_get_permalink('dashboard'); ?>" class="felan-button"> <?php esc_html_e('Go to Dashboard', 'felan-framework'); ?> </a>
        <?php endif;
        do_action('felan_after_service_payment_completed');
        ?>
    </div>
</div>