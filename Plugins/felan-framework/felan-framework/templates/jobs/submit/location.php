...
$jobs_location = get_user_meta($user_id, FELAN_METABOX_PREFIX . 'jobs_user_location', true);
	...
	<?php if (!in_array('fields_jobs_location', $hide_jobs_fields)) : ?>
		<div class="form-group col-lg-6">
			...
			<div class="select2-field">
				<select name="jobs_location" class="felan-select2">
					<?php felan_get_taxonomy_location('felan_location', 'felan_state', 'felan-location-state', 'felan-state-country'); ?>
				</select>
			</div>
		</div>
		...
		<?php if (!in_array('fields_map', $hide_jobs_fields)) : ?>
			...
			<div class="form-group col-md-6">
				...
				<input type="text" id="jobs_longtitude" name="felan_longtitude" value="<?php echo esc_attr($lng); ?>" placeholder="<?php esc_attr_e('0.0000000', 'felan-framework') ?>">
			</div>
			<div class="form-group col-md-6">
				<label for="jobs_latitude"><?php esc_html_e('Latitude', 'felan-framework'); ?></label>
				<input type="text" id="jobs_latitude" name="felan_latitude" value="<?php echo esc_attr($lat); ?>" placeholder="<?php esc_attr_e('0.0000000', 'felan-framework') ?>">
			</div>
		<?php endif; ?>
	<?php endif; ?>
</div>