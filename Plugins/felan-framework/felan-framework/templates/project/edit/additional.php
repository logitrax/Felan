...
?>
<div class="project-fields-wrap">
    <div class="project-fields project-additional row block-from" id="project-submit-additional">
        <?php
        $custom_field_project = felan_render_custom_field('project');
        if (count($custom_field_project) > 0) {
            foreach ($custom_field_project as $key => $field) {
                switch ($field['type']) {
                    case 'text':
        ?>
                        <div class="form-group col-lg-6">
                            <label><?php esc_html_e($field['title']); ?></label>
                            <input type="text" id="<?php echo esc_attr($field['id']); ?>" class="form-control" name="<?php echo esc_attr($field['id']); ?>" value="<?php if (isset($project_meta_data[$field['id']])) {
                                                                                                                                                                        echo sanitize_text_field($project_meta_data[$field['id']][0]);
                    ...
                    ?>
                        <div class="form-group col-lg-6">
                            <label><?php esc_html_e($field['title']); ?></label>
                            <input type="url" id="<?php echo esc_attr($field['id']); ?>" class="form-control" name="<?php echo esc_attr($field['id']); ?>" value="<?php if (isset($project_meta_data[$field['id']])) {
                                                                                                                                                                        echo sanitize_text_field($project_meta_data[$field['id']][0]);
                    ...
                    ?>
                        <div class="form-group col-12">
                            <label><?php esc_html_e($field['title']); ?></label>
                            <textarea name="<?php echo esc_attr($field['id']); ?>" rows="6" id="<?php echo esc_attr($field['id']); ?>" class="form-control"><?php if (isset($project_meta_data[$field['id']])) {
                                                                                                                                                                echo sanitize_text_field($project_meta_data[$field['id']][0]);
                                                                                                                                                            } ?></textarea>
                        </div>
                    <?php
                            ...
                            <div class="select2-field">
                                <select name="<?php echo esc_attr($field['id']); ?>" id="<?php echo esc_attr($field['id']); ?>" class="form-control felan-select2">
                                    ...
                                    foreach ($field['options'] as $opt_value) : ?>
                                        <option value="<?php echo esc_attr($opt_value); ?>" <?php if (isset($project_meta_data[$field['id']]) && $project_meta_data[$field['id']][0] == $opt_value) {
                                                                                                echo 'selected';
                    ...
                    case 'image':
                        ...
                        $project_field = get_post_meta($project_data->ID, $field['id'], true);
                        $custom_image_id = !empty($project_field['id']) ? $project_field['id'] : '';
                        $custom_image_url = !empty($project_field['url']) ? $project_field['url'] : '';
                    ?>
        ...
        <?php
                ...
                case 'file':
                    ...
                    $file_title = '';
                    if(!empty($project_field)){
                        $file_title = get_the_title(intval($project_field));
                    }
                    ?>
                        ...
                        <div class="felan-field-<?php echo esc_attr($field['id']); ?>">
                            ...
                            <div id="felan_cv_plupload_container" class="file-upload-block preview" style="display: inline-block;">
                                <div class="felan_cv_file felan_add-cv">
                                    <p class="drop-cv-<?php echo esc_attr($field['id']); ?>" id="felan_drop_cv" data-attachment-id="<?php echo esc_attr($project_field);?>">
                                        <button class="felan-button" type="button" id="felan_select_cv">
                                            ...
                                            <span>
                                                ...
                                                <?php else: ?>
                                                    <?php esc_html_e('Browse', 'felan-framework'); ?>
                                                <?php endif; ?>
                                            </span>
                                        </button>
                                    </p>
