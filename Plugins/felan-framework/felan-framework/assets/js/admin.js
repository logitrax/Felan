(function ($) {
  ...
  $("body").on("click", ".verify-content-wrap a.verify-button", function (event) {
    event.preventDefault();
	var _this = $(this);
  ...
  $("body").on("click", ".verify-content-wrap a.cancel-verify-button", function (event) {
    event.preventDefault();
	var _this = $(this);
    var user_id = _this.data("user-id");
    $.ajax({
      ...
      url: ajax_url,
      data: {
        action: "felan_cancel_verifications",
        user_id: user_id,
      },
      beforeSend: function () {
        // $(".popup-import .progress-bar-container").removeClass("done");
	...
	$('.verify-view').on('click', function(e){
		...
		var appendDiv = me.next().find('.verify-content-append');
		appendDiv.html('');
		if(user_id != ''){
			me.next().fadeIn();
			$.ajax({
				...
				type      : "POST",
				data      : {
					'action'  : 'felan_user_verifications_info',
					'user_id' : user_id,
				},
				dataType  : "json",
				complete  : function (res) {
					...
					if(data.status){
						appendDiv.html(data.html);
					}
				},
			});
		}
	});
	$('.verify-close').on('click', function () {
		$('.verify-content-wrap').fadeOut();
	});
