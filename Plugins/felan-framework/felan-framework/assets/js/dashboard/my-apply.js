(function ($) {
  ...
  var my_apply = $(".felan-my-apply");

  var ajax_url = felan_my_apply_vars.ajax_url,
    not_jobs = felan_my_apply_vars.not_jobs;
  ...
  $(document).ready(function () {
    ...
    my_apply.find("select.search-control").on("change", function () {
      my_apply.find(".felan-pagination").find('input[name="paged"]').val(1);
      ajax_load_apply();
    });
    ...
    $("body").on(
      "click",
      ".felan-my-apply .jobs-control .btn-delete",
      function (e) {
        e.preventDefault();
    ...
    $("body").on(
        "click",
        ".felan-my-apply .jobs-control .btn-action-review",
        function () {
            var company_id = $(this).attr("employer-id");
    ...
    $("body").on(
      "click",
      ".felan-my-apply .felan-pagination a.page-numbers",
      function (e) {
        e.preventDefault();
        my_apply
        ...
        var current_page = 1;
        if (
          my_apply.find(".felan-pagination").find('input[name="paged"]').val()
        ) {
    ...
    function ajax_load_apply(item_id = "", action_click = "") {
      ...
      var jobs_search = my_apply.find('input[name="jobs_search"]').val(),
        ...
        paged = my_apply.find('.felan-pagination input[name="paged"]').val();

      $.ajax({
        dataType: "json",
        url: ajax_url,
        data: {
          action: "felan_filter_my_apply",
        ...
        beforeSend: function () {
          my_apply.find(".felan-loading-effect").addClass("loading").fadeIn();
