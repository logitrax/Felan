...
(function ($) {
  ...
  var ajax_url = felan_template_vars.ajax_url,
    ...
    item_amount = felan_project_archive_vars.item_amount,
    not_project = felan_project_archive_vars.not_project,
    range_min = felan_project_archive_vars.range_min,
    range_max = felan_project_archive_vars.range_max,
    currency_sign = felan_project_archive_vars.currency_sign;

  var ajax_call = false;

  var menu_filter_wrap = $(".felan-menu-filter");
  var archive_project = $(".archive-project");
  var felan_map;
  var markers = [];
  //map
  ...
  if (mapType == "google_map") {
    var project_maps_filter = $("#jobs-map-filter");
  } else if (mapType == "openstreetmap") {
    var project_maps_filter = $("#maps");
  } else {
    var project_maps_filter = $("#map");
  }

  if (project_maps_filter.length) {
    has_map = "yes";
  }
  ...
  var felan_hover_map_effects = function () {
    if (map_effects !== "" && has_map) {
      $(".map-event .area-project .felan-project-item").each(function () {
        var title = $(this).find(".btn-add-to-wishlist").data("project-id");
  ...
  PROJECT.elements = {
    init: function () {
      ...
      archive_project
        ...
        .on("input", "input.input-control", function () {
          ...
          $(".form-project-top-filter .btn-top-filter").removeData("clicked");
          ajax_call = true;
          PROJECT.elements.ajax_load(ajax_call);
        });

      archive_project.find("select.sort-by").on("change", function () {
        $(".felan-pagination").find('input[name="paged"]').val(1);
        $(".form-project-top-filter .btn-top-filter").removeData("clicked");
        ajax_call = true;
        PROJECT.elements.ajax_load(ajax_call);
      ...
      menu_filter.find('input[name="project_filter_price_min"]').keyup(
        delay(function () {
          $(".felan-pagination").find('input[name="paged"]').val(1);
      ...
      menu_filter.find('input[name="project_filter_price_max"]').keyup(
        delay(function () {
          $(".felan-pagination").find('input[name="paged"]').val(1);
          $(".form-jobs-top-filter .btn-top-filter").removeData("clicked");
          ajax_call = true;
      ...
      menu_filter
        .find('select[name="project_time_type"]')
        .on("change", function () {
          $(".felan-pagination").find('input[name="paged"]').val(1);
          $(".form-jobs-top-filter .btn-top-filter").removeData("clicked");
          ajax_call = true;
      ...
      archive_project
        .find(".form-project-top-filter .btn-top-filter")
        .on("click", function (e) {
          e.preventDefault();
          $(".felan-pagination").find('input[name="paged"]').val(1);
      ...
      if (project_maps_filter.length > 0) {
        PROJECT.elements.ajax_load();
      }
      ...
      $("body").on(
        ...
        function (e) {
          ...
          if ($(this).attr("data-type") == "number") {
            PROJECT.elements.scroll_to(".area-project");
            PROJECT.elements.ajax_load(ajax_call);
          } else {
            PROJECT.elements.ajax_load(ajax_call, "loadmore");
          }
        }
      );
    },
    ...
    project_layout: function () {
      archive_project.find(".project-layout a").on("click", function (event) {
        ...
        $(this).closest(".project-layout").find(">a").removeClass("active");
        $(this).addClass("active");
        PROJECT.elements.removeClassStartingWith(
        ...
        $(".area-project .felan-project-item").each(function () {
          PROJECT.elements.removeClassStartingWith($(this), "layout-");
        ...
        $("#slider-range").slider({
            ...
            change: function (event, ui) {
                ...
                if (values_start !== min || values_end !== max) {
                    archive_project.addClass("filter-active");
                } else {
                    archive_project.removeClass("filter-active");
                }
            },
            stop: function () {
                delayShowData();
    ...
    display_clear: function () {
      ...
      if ($(".felan-menu-filter ul.filter-control li.active").length > 0) {
        ...
        archive_project.find(".felan-clear-filter").show();
      } else {
        $(".felan-nav-filter").removeClass("active");
        archive_project.find(".felan-clear-filter").hide();
      }

      if (
        $('.felan-menu-filter input[name="project_filter_price_min"]').val() !==
          "" ||
        $('.felan-menu-filter input[name="project_filter_price_max"]').val() !==
          "" ||
        $('.felan-menu-filter select[name="project_time_type"]').val() !== ""
      ) {
        archive_project.find(".felan-clear-filter").show();
      } else {
        archive_project.find(".felan-clear-filter").hide();
    ...
    filter_clear_top: function () {
      archive_project.find(".felan-clear-top-filter").on("click", function () {
        $('.form-project-top-filter input[name="project_filter_search').val("");
        $('.form-project-top-filter input[name="project-search-location"]').val(
          ""
        );
        $(".form-project-top-filter .felan-select2").val("");
        $(".form-project-top-filter .felan-select2").select2("destroy");
        $(".form-project-top-filter .felan-select2").each(function () {
    ...
    filter_clear: function () {
      archive_project.find(".felan-clear-filter").on("click", function () {
    ...
    ajax_load: function (ajax_call, pagination) {
      ...
      paged = archive_project
        .find(".felan-pagination")
        .find('input[name="paged"]')
        .val();
      title = archive_project.find('input[name="project_filter_search"]').val();
      current_term = $('input[name="current_term"]').val();
        has_map_val = $('input[name="has_map"]').val();
        type_term = $('input[name="type_term"]').val();

      project_layout = archive_project
        .find(".project-layout a.active")
        .attr("data-layout");
      ...
      location = $('input[name="project-search-location"]').val();
      ...
      radius_cities = $(".felan-form-location")
        .find('input[name="project_number_radius"]')
        .val();

      if (result_fields.hasOwnProperty("project-categories")) {
        categories = $('input[name="project-categories_id[]"]:checked')
          .map(function () {
            return $(this).val();
          })
          .get();
      } else {
        categories = $('select[name="project-categories"]').val();
      }

      if (result_fields.hasOwnProperty("project-skills")) {
        skills = $('input[name="project-skills_id[]"]:checked')
          .map(function () {
            return $(this).val();
          })
          .get();
      } else {
        skills = $('select[name="project-skills"]').val();
      }

      if (result_fields.hasOwnProperty("project-language")) {
        language = $('input[name="project-language_id[]"]:checked')
          .map(function () {
            return $(this).val();
          })
          .get();
      } else {
        language = $('select[name="project-language"]').val();
      }
      ...
      if (result_fields.hasOwnProperty("project-rating")) {
        rating = $('input[name="project_rating[]"]:checked')
          .map(function () {
            return $(this).val();
          })
          .get();
      } else {
        rating = $('select[name="project-rating"]').val();
      }
      ...
      if (mapType == "google_map") {
        ...
        var felan_add_markers = function (props, map) {
          $.each(props, function (i, prop) {
            ...
            contentString.innerHTML = prop.project;
            ...
            google.maps.event.addListener(marker, "click", function () {
              ...
              var elem = $(marker.url);
              $(".area-project .felan-project-item").removeClass("highlight");
              if (
                elem.length > 0 &&
                click_marker &&
                $(".archive-project.map-event").length > 0
              ) {
          ...
          function handleLocationError(browserHasGeolocation, infowindow, pos) {
            ...
            infowindow.setContent(
              browserHasGeolocation
                ? "Error: The Geolocation project failed."
        ...
        var felan_osm_add_markers = function (props, maps) {
          ...
          function addMarkers() {
            ...
            stores.features.forEach(function (marker) {
              ...
              if (map_effects == "popup") {
                markers.bindPopup(marker.properties.project);
              } else {
                markers.bindPopup();
              }
        ...
        var felan_mapbox_add_markers = function (props, map) {
          ...
          map.on("load", function (e) {
            ...
            map.addLayer({
              id: "locations",
              type: "symbol",
              /* Add a GeoJSON source containing project coordinates and information. */
              source: {
                type: "geojson",
                data: stores,
              },
          ...
          function createPopUp(currentFeature) {
            ...
            var popup = new mapboxgl.Popup({ closeOnClick: false })
              ...
              .setHTML(currentFeature.properties.project)
              .addTo(map);
          }
      ...
      PROJECT.elements.display_clear();
      var type_pagination = $(".felan-pagination").attr("data-type");
      $(".area-project .felan-project-item").addClass("skeleton-loading");

      $.ajax({
        dataType: "json",
        url: ajax_url,
        data: {
          action: "felan_project_archive_ajax",
          paged: paged,
          title: title,
        ...
        beforeSend: function () {
          ...
          if (
            ...
            archive_project.find(".btn-top-filter .btn-loading").fadeIn();
          }
          if (type_pagination == "loadmore") {
            archive_project.find(".btn-loading").fadeIn();
          }
        },
        success: function (data) {
          archive_project.find(".btn-top-filter .btn-loading").fadeOut();
          archive_project
            .find(".felan-filter-search-map .felan-loading-effect")
            .fadeOut();
          $(".area-project .felan-project-item").removeClass(
            "skeleton-loading"
          );
          ...
          if (data.success === true) {
            if (ajax_call == true) {
              if (
                ...
                archive_project.find(".area-project").html(data.project_html);
                archive_project
                  .find(".felan-pagination .pagination")
                  .html(data.pagination);
                archive_project.find(".result-count").html(data.count_post);
              } else {
                archive_project.find(".area-project").append(data.project_html);
                if (data.hidden_pagination) {
                  archive_project
                    .find(".felan-pagination .pagination")
                    .html("");
                }
                archive_project.find(".btn-loading").fadeOut();
                archive_project
                  .find(".felan-pagination .pagination")
                  .removeClass("active");
              }
            ...
            if (ajax_call == true) {
              if (
                ...
                archive_project
                  .find(".area-project")
                  .html(
                    '<div class="felan-ajax-result">' + not_project + "</div>"
                  );
                archive_project.find(".result-count").html(data.count_post);
                archive_project.find(".felan-pagination .pagination").html("");
              } else {
                archive_project.find(".area-project").append(data.project_html);
                if (data.hidden_pagination) {
          ...
          if (has_map) {
            if (mapType == "google_map") {
              ...
              markers = [];
              felan_add_markers(data.project, felan_map);
              ...
              felan_osm_add_markers(data.project, maps);
            } else {
              felan_mapbox_add_markers(data.project, map);
            }
          }
        },
      });
  ...
  $(window).load(PROJECT.onLoad.init);
})(jQuery);
