(function ($) {
  ...
  var ajax_url = felan_template_vars.ajax_url,
    ...
    not_company = felan_company_archive_vars.not_company,
    map_api_key = felan_template_vars.map_api_key,
    item_amount = felan_company_archive_vars.item_amount,
    range_min = felan_company_archive_vars.range_min,
    range_max = felan_company_archive_vars.range_max;
  ...
  var felan_hover_map_effects = function () {
    if (map_effects !== "" && has_map) {
      $(".map-event .area-company .felan-company-item").each(function () {
  ...
  COMPANY.elements = {
    ...
    company_layout: function () {
      archive_company.find(".company-layout a").on("click", function (event) {
        ...
        $(".area-company .felan-company-item").each(function () {
    ...
    display_clear: function () {
      ...
      if ($(".felan-menu-filter ul.filter-control li.active").length > 0) {
        ...
        archive_company.find(".felan-clear-filter").show();
      } else {
        $(".felan-nav-filter").removeClass("active");
        archive_company.find(".felan-clear-filter").hide();
      }
    ...
    filter_clear_top: function () {
      archive_company.find(".felan-clear-top-filter").on("click", function () {
        ...
        $(".form-company-top-filter .felan-select2").val("");
        $(".form-company-top-filter .felan-select2").select2("destroy");
		$('input[name="current_term"]').val("");
		$('input[name="type_term"]').val("");
        $(".form-company-top-filter .felan-select2").each(function () {
    ...
    filter_clear: function () {
      archive_company.find(".felan-clear-filter").on("click", function () {
        $(".felan-menu-filter ul.filter-control li").removeClass("active");
        $('.felan-menu-filter input[type="checkbox"]').prop("checked", false);
        $('.felan-menu-filter select[name="company_filter_salary_min"]').prop(
          "selectedIndex",
          0
        );
        $('.felan-menu-filter select[name="company_filter_salary_max"]').prop(
          "selectedIndex",
          0
        );
        $('.felan-menu-filter select[name="company_filter_rate"]').prop(
          "selectedIndex",
          0
        );
		...
		$('input[name="company-search-location"]').val("");
		$('input[name="current_term"]').val("");
		$('input[name="type_term"]').val("");
		$('select[name="company-categories"]').val("");
		$('select[name="company-categories"]').select2("destroy");
		$('select[name="company-categories"]').select2();
        ajax_call = true;
        COMPANY.elements.ajax_load(ajax_call);
      });
    },
    ...
    ajax_load: function (ajax_call, pagination) {
      ...
      radius_cities = $(".felan-form-location")
        .find('input[name="company_number_radius"]')
        .val();
      ...
      if (result_fields.hasOwnProperty("company-categories")) {
        categories = $('input[name="company-categories_id[]"]:checked')
          .map(function () {
            return $(this).val();
          })
          .get();
      ...
      if (result_fields.hasOwnProperty("company-size")) {
        size = $('input[name="company-size_id[]"]:checked')
          .map(function () {
            return $(this).val();
          })
          .get();
      ...
      if (result_fields.hasOwnProperty("company-rating")) {
        rating = $('input[name="company_rating[]"]:checked')
          .map(function () {
            return $(this).val();
          })
          .get();
        ...
        var felan_add_markers = function (props, map) {
          $.each(props, function (i, prop) {
            ...
            google.maps.event.addListener(marker, "click", function () {
              ...
              var elem = $(marker.url);
              $(".area-company .felan-company-item").removeClass("highlight");
              if (
                elem.length > 0 &&
                click_marker &&
      ...
      $.ajax({
        ...
        success: function (data) {
            if(data.url_push == '1'){
                var searchQuery = $('#company_filter_search').val();
            ...
            if (ajax_call == true) {
              if (
                ...
                archive_company
                  .find(".area-company")
                  .html(
                    '<div class="felan-ajax-result">' + not_company + "</div>"
                  );
                archive_company.find(".result-count").html(data.count_post);
                archive_company.find(".felan-pagination .pagination").html("");
              } else {
                archive_company.find(".area-company").append(data.company_html);
                if (data.hidden_pagination) {
          ...
          if (has_map) {
            if (mapType == "google_map") {
              ...
              markers = [];
              felan_add_markers(data.company, felan_map);
              ...
              felan_osm_add_markers(data.company, maps);
            } else {
              felan_mapbox_add_markers(data.company, map);
            }
          }
